...
- [x] **13. Joining a round is a read-modify-write race; nobody ever leaves** (`PlayRoute.js:89-103`)
  ...
  - [!] `firestore.rules` changed on this branch (the old self-join clause is gone; `selfJoinOrLeave()` now covers the join and leave writes the client makes) and must be deployed before leaving/joining works against production — the merge workflow only deploys hosting, not rules.
