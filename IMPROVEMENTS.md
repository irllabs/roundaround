...
- [x] **13. Joining a round is a read-modify-write race; nobody ever leaves** (`PlayRoute.js:89-103`)
  ...
  - [!] `firestore.rules` changed on this branch (the old self-join clause is gone; `selfJoinOrLeave()` now covers the join and leave writes the client makes) and the merge workflow only deploys hosting, not rules. Until the rules are deployed, the client falls back to two writes for any round that rejects the single two-field one: the old one-field join (`currentUsers` only), then a contributors-only union for the same user, which those rules accept because the first write has just made them a member. Nobody is locked out either way, and if the second write is refused as well the store keeps the contributors the document has, so the entry is simply filled in on a later visit. Run `firebase deploy --only firestore:rules` to get leaving and joining working properly, and to close the old rule that let any signed-in user rewrite `currentUsers`.
...
- [~] **34.** Zero unit tests: now 223 Vitest/Testing Library tests across 20 files (`yarn test`) covering the data layer, sample loading, PlayRoute subscriptions, the dialogs and the slider regression; CI runs them. Added since: oracle tests for every reducer (`round.test.js`, `display.test.js`, `rounds.test.js`, `user.test.js`, `users.test.js`), PlayUI's debounced pattern saving (`PlayUI.test.jsx`), the round diff PlayUI redraws from (`roundDiff.test.js`), and the audio graph's track, effects and sampled instruments (`Track.test.js`, `fx.test.js`, `sampleInstrument.test.js`). Still uncovered: PlayUI's drawing, which needs a real SVG.js, and the audio engine itself. The Cypress smoke test now runs against a local build in CI.
