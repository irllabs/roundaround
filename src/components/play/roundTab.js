...
export function tabGeometry({ cx, cy, ringRadius, bandWidth, gap, text, textWidth, offsetDeg = 0 }) {
    ...
    return {
        ...
        startAngle,
        endAngle,
        labelPath: tagPath(cx, cy, inner, outer, startAngle, endAngle, Math.min(CORNER, height / 2, width / 4)),
...
/**
 * A tag standing on a ring: flat on the ring's outer edge (an arc of radius `inner`), straight
 * radial sides, a top that follows the ring at `outer`, and rounded top corners of radius `corner`.
 */
function tagPath(cx, cy, inner, outer, a0, a1, corner) {
    ...
    return [
        ...
        `A${outer} ${outer} 0 ${a1 - a0 > 180 ? 1 : 0} 1 ${P(outer, a1 - cornerDeg)}`,
        `Q${P(outer, a1)} ${P(outer - corner, a1)}`,
        `L${P(inner, a1)}`,
        `A${inner} ${inner} 0 ${a1 - a0 > 180 ? 1 : 0} 0 ${P(inner, a0)}`,
        'Z',
    ].join(' ')
}
